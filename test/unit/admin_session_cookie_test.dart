...
void main() {
  group('parseAdminSessionCookie', () {
    test(
      'happy path — returns the session cookie unchanged through "name=value"',
      () {
    ...
    test(
      'preserves cookies with rich values (Base64 / dashes / equals padding)',
      () {
    ...
    test(
      'picks AIRWATCH_ADMIN_SID even when other cookies share the response',
      () {
        final out = parseAdminSessionCookie(
          location: '/admin/overview',
          setCookieHeaders: const [
    ...
    test('empty inputs are safe (no exceptions)', () {
      expect(
        parseAdminSessionCookie(location: '', setCookieHeaders: const []),
        isNull,
      );
    });
  });
