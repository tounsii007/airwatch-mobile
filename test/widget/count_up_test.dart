...
void main() {
  group('CountUp widget', () {
    testWidgets(
      'renders the initial value formatted with thousands separator',
      (tester) async {
    ...
    testWidgets('decimals=2 keeps two trailing fraction digits', (
      tester,
    ) async {
    ...
    testWidgets('value change tweens — final frame matches new value', (
      ...
      await tester.pumpWidget(
        _wrap(const CountUp(value: 100, duration: Duration(milliseconds: 100))),
      );
      // Update to a new value — should animate over 100 ms.
      await tester.pumpWidget(
        _wrap(const CountUp(value: 200, duration: Duration(milliseconds: 100))),
      );
    ...
    testWidgets('survives rapid value changes without crashing', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap(const CountUp(value: 0)));
      for (var i = 1; i <= 5; i++) {
