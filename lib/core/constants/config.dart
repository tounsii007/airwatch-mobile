...
///
/// The mobile app is a thin client on top of `airwatch-api`. Any
/// third-party API that the web frontend reaches via the Next.js
/// rewrite (Airlabs, hexdb, Planespotters, Open-Meteo) is reached from
/// mobile through the same backend endpoints — the mobile binary never
/// embeds an API key.
///
...
/// ```
///
/// Release-build safety: [apiBaseUrl] **throws** in `kReleaseMode` if no
/// `API_BASE_URL` is supplied or if it doesn't start with `https://`.
/// This prevents an accidental release build from shipping with the
/// localhost dev fallback (which would also auto-disable certificate
/// pinning, see [CertificatePinning]). Debug builds keep the lenient
/// fallback so `flutter run` works without ceremony.
library;
...
class AppConfig {
  ...
  /// Resolution order:
  ///
  /// 1. `--dart-define=API_BASE_URL=...` at build time (prod).
  /// 2. Debug fallback to `http://localhost:18090` for `flutter run`.
  ///
  /// In release mode, missing or non-HTTPS URLs throw [StateError] —
  /// the runZonedGuarded handler in `main.dart` catches it and the app
  /// refuses to start, which is far better than silently sending admin
  /// credentials in cleartext.
  static const String _definedBase = String.fromEnvironment('API_BASE_URL');

  static String get apiBaseUrl {
    if (_definedBase.isNotEmpty) {
      if (kReleaseMode && !_definedBase.startsWith('https://')) {
        throw StateError(
          'AppConfig.apiBaseUrl: release builds require an https:// URL '
          '(got "$_definedBase"). Pass '
          '--dart-define=API_BASE_URL=https://your-host '
          'when building. Cleartext HTTP also auto-disables certificate '
          'pinning, which would expose admin credentials to MITM.',
        );
      }
      return _definedBase;
    }
    // Release builds with no API_BASE_URL: fail closed. Anyone shipping
    // a binary without an explicit prod URL is making a mistake.
    if (kReleaseMode) {
      throw StateError(
        'AppConfig.apiBaseUrl: API_BASE_URL is required for release '
        'builds. Pass --dart-define=API_BASE_URL=https://your-host.',
      );
    }
    // Debug fallback only — Hits the API's MOBILE port (18090). The
    // web port (18080) and the management port (19091) reject mobile
    // clients with 404 (web), or are bound to loopback only
    // (management). See MultiPortServerConfig in airwatch-api for the
    // per-port allow-list.
  ...
  static String scheduleByAirlineUrl({
    String? airlineIcao,
    String? airlineIata,
  }) {
