...
class _AirportScreenState extends ConsumerState<AirportScreen> {
  ...
  void _onSearch(String query) {
    ...
    // Resolve a locale-typed country query ("Tunesien" / "Tunisie") to its
    // canonical ISO code so we can also surface every airport in that
    // country, mirroring the web frontend's airport search behaviour.
    // `find` already handles the "USA"/"UK" English aliases and falls
    // through to the de/fr `country_translations.dart` reverse-lookup.
    final countryHit = CountryDatabase.find(raw);
    final countryCode = countryHit?.code.toUpperCase();

    final results = airportFullDatabase.values
        .where((a) {
          ...
          // Country-code match — covers a typed country name in any locale.
          if (countryCode != null && a.country.toUpperCase() == countryCode) {
            return true;
          }
          // Substring matches across IATA / ICAO / name / city in English.
          if (a.iata.toUpperCase().contains(q) ||
              a.icao.toUpperCase().contains(q) ||
              a.name.toUpperCase().contains(q) ||
              a.city.toUpperCase().contains(q)) {
            return true;
          }
          // Locale-aware city match — typing "Köln" finds CGN, "Nizza" finds
          // NCE, "Mailand" finds MXP. Same helper the web's `filterAirports`
          // uses, lazily seeded on first call.
          return cityNameMatches(a.city, raw) || cityNameMatches(a.name, raw);
        })
        .take(15)
        .toList();
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _localizeAirportName(context, apt.name),
                          style: TextStyle(
                            fontFamily: UiConstants.bodyFont,
                            fontSize: 14,
                            fontWeight: FontWeight.w700,
  ...
  Widget _buildDefaultView(
    bool isDark,
    Color primary,
    AsyncValue<Map<String, AircraftState>> aircraftAsync,
    ...
    return aircraftAsync.when(
      data: (aircraft) {
        ...
        return ListView(
          ...
          children: [
            ...
            Row(
              children: [
                _Stat(
                  ...
                  onTap: () => setState(
                    () =>
                        _activeFilter = _activeFilter == _FlightFilter.airborne
                        ? _FlightFilter.all
                        : _FlightFilter.airborne,
                  ),
                ),
                ...
                _Stat(
                  ...
                  onTap: () => setState(
                    () => _activeFilter = _activeFilter == _FlightFilter.ground
                        ? _FlightFilter.all
                        : _FlightFilter.ground,
                  ),
                ),
                const SizedBox(width: 8),
                _Stat(
                  ...
                  onTap: () =>
                      setState(() => _activeFilter = _FlightFilter.all),
                ),
              ],
            ),
            ...
            if (_activeFilter == _FlightFilter.all ||
                _activeFilter == _FlightFilter.airborne) ...[
              Text(
                context.tr('recent_departures'),
            ...
            if (_activeFilter == _FlightFilter.ground) ...[
              Text(
                context.tr('on_ground').toUpperCase(),
                style: const TextStyle(
                  fontFamily: UiConstants.headingFont,
                  fontSize: 10,
                  fontWeight: FontWeight.w700,
      ...
      error: (_, _) => Center(
        child: Text(
          context.tr('unable_to_load_data'),
          style: TextStyle(
...
class _FlightTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final altColor = altFt < AppConfig.altitudeLowMax
        ? AppColors.altitudeLow
        : altFt < AppConfig.altitudeMedMax
          ...
          children: [
            ...
            Column(
              ...
              children: [
                if (alt != null)
                  Text(
                    UiText.formatAltitude(context, settings, alt),
                    style: TextStyle(
                      fontFamily: UiConstants.headingFont,
                      fontSize: 10,
                      fontWeight: FontWeight.w700,
                ...
                if (aircraft.velocity != null)
                  Text(
                    UiText.formatSpeed(context, settings, aircraft.velocity),
                    style: TextStyle(
                      fontFamily: UiConstants.bodyFont,
                      fontSize: 11,
                      color: primary,
